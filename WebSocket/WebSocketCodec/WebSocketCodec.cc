...
frame::frame(char fin, char opcode, char mask, uint64_t payloadLen, char *maskbuf, string messege)
    :fin_(fin),
    opcode_(opcode),
    mask_(mask),
...
uint64_t frame::getPayloadLen()
{
    return payloadLen_;
}

//包装为一个完整帧，可直接发送
string frame::toFrame(string &messege, char opcode)
{
    char buf[128] = {0};
    int buflen = 0;
    buf[0] = 0b10000000;
...
string frame::toFrameWithMask(string &messege, char opcode, string maskstr )
{
    char buf[128] = {0};
    char maskbuf[4];
    int buflen = 0;
...
WebSocketCodec::WebSocketCodec(Buffer *buf)
    :frames(),
    buf_(buf),
    kfin(true),
    klenfin(true),
...
bool WebSocketCodec::isWebSocket(requestHeader *header)
{
...
//错误返回空串
string WebSocketCodec::toHttpResponse(requestHeader *header)
{
    string reqkey_;
...
bool WebSocketCodec::parseFrame()
{
    //处理请求body不完整的情况
    if (kfin == false)
    {
